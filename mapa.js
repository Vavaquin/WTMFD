...
function format_mission_data(mission) {
  ...
  if ('objectives' in mission && mission['objectives'] != null) {
...
function calcMapObjectColor(item) {
  if (('blink' in item) && (item['blink'])) {
    ...
    var c0 = item['color[]']
    var c1 = [255, 0, 0]
    var c = rgb_to_hex(c0[0], c0[1], c0[2]);
    return c
  } else {
    return item['color']
  }
...
function drawMapGridScaled(canvas) {
  ...
  ctx.font = 'normal 10pt Electrolize'
...
function add_to_chat(root, rec) {
  ...
  if ("time" in rec)
  {
    ...
    elTime.text(format_time(rec.time))
    elTime.addClass('chat-time')
    el.append(elTime)
  }
...
//   $.ajax({type:'GET', url:'http://localhost:8111/hudmsg', data:{'lastEvt':lastEvtMsgId, 'lastDmg':lastDmgMsgId}, success:update_hud_msg })
//   $.ajax({type:'GET', url:'http://localhost:8111/indicators',    success:update_indicators })
//   $.ajax({type:'GET', url:'http://localhost:8111/state',         success:update_state,
...
function local_debug_init() {
  ...
  for (var n=0; n<5; ++n) {  
    for (var i=0; i<dbg_chat.length; ++i) {
      add_to_chat(root, {'id':i+1, 'msg': dbg_chat[i], 'sender': 'Rick', 'enemy': false, 'mode': dbg_mode[i], 'time': n * 100 + i})
    }
  }
  root.get(0).scrollTop = root.get(0).scrollHeight
}
